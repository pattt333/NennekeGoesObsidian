...
When all tasks are complete, run `openspec validate --change "<name>"` plus the task-specific checks. Review `git diff`, stage only files created or modified for this change, and automatically commit them with a concise imperative Conventional Commit message. Never include unrelated pre-existing edits. Report validation results and the commit hash, then suggest `/opsx:archive`.
