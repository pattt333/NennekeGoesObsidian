...
- Replace the fixed PDF input list with a versioned publication manifest that includes the current chapter scope plus an expandable, explicitly selected set of spells and liturgies.
...
- `configurable-rulebook-publication`: Build a PDF from a declarative publication manifest with selected, extensible spell and liturgy content.
