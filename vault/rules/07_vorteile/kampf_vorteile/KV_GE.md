<!-- Source: Chapters/07_vorteile/kampf_vorteile/KV_GE.tex -->

### GE-Vorteile

| **Voraussetzung** | **GE** |
| --- | --- |
| **Attribut 4** |  |
| **20EP** | **Standfest** Halbiert die Erschwerniss durch [unvorteilhafte und unterlegende Position im Nahkampf](../../06_kampf/Angriff_Verteidigung.md#subsec-modsimkampf). |
| **Attribut 6** |  |
| **40EP** | **Sturmangriff** Erlaubt das Manöver [Sturmangriff](../../06_kampf/kampfmanover/Nahkmapfspezialmanover.md#subsubsec-sturmangriff). |
| **Attribut 8** |  |
| **60EP** | **Todesstoß** Erlaubt das Manöver [Todesstoß](../../06_kampf/kampfmanover/Nahkmapfspezialmanover.md#subsubsec-todesstoss). |
| **Attribut 10** |  |
| **80EP** | **Präzision** Zeigt der gewertete Würfel bei einem Angriff eine 16 oder höher, richtet der Angriff +GE TP zusätzlich an. |
