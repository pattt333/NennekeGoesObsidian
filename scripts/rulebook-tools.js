...
function buildPdf(root = ROOT) {
  ...
  const result = spawnSync("pandoc", [intermediate, "--from=markdown+raw_html", "--lua-filter", PDF_TYPOGRAPHY_FILTER, "--lua-filter", HEALTH_SEGMENT_FILTER, "--lua-filter", RULE_EXAMPLE_FILTER, "--output", output, "--toc", "--number-sections", "--top-level-division=chapter", `--pdf-engine=${config.pdfEngine}`, "--metadata", `title=${config.title}`, "--metadata", "lang=de-DE"], { cwd: root, encoding: "utf8" });
